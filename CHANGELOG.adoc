...
* *Plugin Activation Gating (`get_plugin_manager()`):* Refactored to a two-pass discovery → registration architecture. `config.plugins` is now the sole authority on which plugins run and in what order — presence in a discovery source (installed entry point or local `plugins/` directory) alone no longer activates a plugin. List order controls execution order.

...
| Documentation for bundled plugins: git source links, navigation breadcrumbs, back-of-the-book index and glossary, API doc generation, and doctest verification.
...
[glossary]
AsciiDoc::
  A lightweight markup language for writing technical documentation. Golem uses AsciiDoc as its primary source format, parsed via AsciiDoctrine.
Golem::
  A static-site documentation compiler built on AsciiDoctrine, Chameleon, and Pluggy. Golem transforms AsciiDoc source trees into HTML sites with a pluggable pipeline.

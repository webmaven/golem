...
- [x] Task 7: Index & Glossary Compilation Plugins (Issue #5)
...
- [x] Task 6: Navigation Helpers Plugin (commit 231b138, review clean)
...
- [x] Task 7: Index & Glossary Compilation Plugins (commits 9d86c61 + 850c45d, review clean)
  Key outcome: `IndexPlugin` + `GlossaryPlugin` + `IndexGlossaryPlugin` (unified) via `on_asg_created`. Accumulates `IndexTerm` nodes (primary/secondary/tertiary) and `[glossary]`-role `DescriptionList` entries across all docs. `compile_index()` / `compile_glossary()` return alphabetized nested dicts with location lists. Dual walker supports typed Node trees and raw ASG dicts. `reset()` for fresh builds. 17 new tests; 440 total passing.
